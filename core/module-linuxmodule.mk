...
@mkdir -p $(@D)
@sed -e 's/MODULE_DESCRIPTION[ ]*[(]/MODULE_DESCRIPTION("$$Attr: app.name=$(APPNAME) $$ $$Attr: app.version=$(VERSION) $$ $$Attr: app.revision=$(REVISION) $$ $$Attr: build.mode=$(MODE) $$ $$Attr: build.opts=$(DEFINES) $$Attr: build.date='`date +%Y-%m-%d.%H:%M:%S`' $$ $$Attr: build.host='`hostname`' $$ $$Attr: build.user='`whoami`' $$ $$Attr: build.id=$(BUILDNUM) $$ \\n\\t\\t\" /g' $^ > $@
endef
