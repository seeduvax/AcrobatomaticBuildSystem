...
$(CARGO_TOML): $(CARGO_TOML_TEMPLATE) module.cfg $(PRJROOT)/app.cfg
	...
	@VERSION=`echo $(VERSION) | sed 's/[a-zA-Z]//g'` && \
		grep -E -v "^#.*" $< | sed "s/@VERSION@/$$VERSION/g" | \
		sed -E "s~@DEPENDENCIES@~$(RUST_DEPENDS_TEXT)~g" >> $@.tmp
	@sed -i -E "s/@RUST_EDITION@/$(RUST_EDITION)/g" $@.tmp
	@sed -i -E "s/@CREATE_TYPE@/$(subst $(_space_),$(_comma_),$(patsubst %,'%',$(CRATETYPE)))/g" $@.tmp
	@chmod +w $@
	@chmod -w $@.tmp
	@mv $@.tmp $@
