...
## ------------------------------------------------------------------------
ALLINCLUDES_MK=$(PRJOBJDIR)/.allExtDepsInclude
...
EXTLIBMAKE_ALL_LIBS=$(USELIB) $(NDUSELIB) $(NDNA_USELIB) $(NA_USELIB)
EXTLIBMAKE_FULL_IMPORTED:=$(patsubst %,$(PRJOBJDIR)/.%_fullyImported,$(subst |,-,$(EXTLIBMAKE_ALL_LIBS)))

# use allinclude.mk to be able to get all dependencies first.
$(ALLINCLUDES_MK): $(PRJROOT)/app.cfg
	@mkdir -p $(@D)
	@$(ABS_PRINT_debug) "Getting all dependencies"
	@+make --no-print-directory getdeps
	@touch $@

$(PRJOBJDIR)/.%_fullyImported: $(ALLINCLUDES_MK)
